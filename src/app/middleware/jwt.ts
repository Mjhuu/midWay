...
module.exports = options => {
    return async function jwt(ctx: Context, next) {
        if (ctx.url !== '/' && !ctx.url.includes('/login') && !ctx.url.includes('/captcha')) {
            ...
            if (!ctx.headers.referer.includes('http://192.168.0.109:3000') && !ctx.headers.referer.includes('http://192.168.0.109:7003') && !ctx.headers.referer.includes('http://192.168.0.79:7003')) {
